...
**SQLite locally, Postgres when the path is a DSN.** A `postgres://` path routes
`connect` through psycopg2 with the placeholders and DDL translated at the seam,
because on a container filesystem a SQLite record dies with the revision, and
this store is the only place a case's history exists.
"""
...
class _PGConn:
    """A psycopg2 connection speaking the sqlite3 shorthand the store was written in.

    The store's SQL is portable except for three dialect points, all folded here:
    `?` placeholders become `%s`, `BEGIN IMMEDIATE` has no Postgres meaning beyond
    `BEGIN`, and enum parameters are flattened to their string value because
    psycopg2 adapts by exact type.
    """
    ...
    def execute(self, sql, params=()):
        ...
        cur.execute(sql.replace("?", "%s").replace("BEGIN IMMEDIATE", "BEGIN"),
                    [str(p) if isinstance(p, StrEnum) else p for p in params])
        return cur
...
class Store:
    ...
    def connect(self):
        if self.pg:
            ...
            conn.autocommit = True   # transactions are the explicit BEGIN/COMMIT below
            db = _PGConn(conn)
        else:
    ...
    def transition(self, case_id: str, to_state: CaseState, actor: str,
        ...
        with self.connect() as db:
            ...
            try:
                ...
                db.execute("ROLLBACK")
                # SQLite's BEGIN IMMEDIATE serializes writers, so the key check
                # above is race-free there. Postgres runs READ COMMITTED: two
                # workers with the same key can both pass the check, and the
                # loser hits the UNIQUE constraint. Same promise, same answer.
